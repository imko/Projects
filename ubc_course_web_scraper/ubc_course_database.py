		...
		for schedule in course.schedules: 
			for day, info in schedule.items(): 
				if info["instructor"] is not None:
					...
					self.c.execute("INSERT INTO ubcCourseTable(term, course_name, course_number, course_section, activity, days, start_time, end_time, instructor, building, room_number) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (course.term, course_name, course_number, course.section, course.activity, day, info["start_time"], info["end_time"], None, info["building"], info["room"]))
		self.conn.commit() 
